...
ExchangeManager::translate_result(const events::OrderMessage& msg, const MatchResult& res,
    ...
    for (const auto& trade : res.trades) {
        ...
        ev.symbol = msg.symbol;
        ev.buy_order_id =
            (trade.aggressor_side == AggressorSide::Buy) ? trade.aggressor_id : trade.passive_id;
        ev.sell_order_id =
            (trade.aggressor_side == AggressorSide::Buy) ? trade.passive_id : trade.aggressor_id;
