...
static const char* side_to_str(Side s) noexcept { return s == Side::Buy ? "BUY" : "SELL"; }
...
static const char* reject_reason_to_str(RejectReason r) noexcept {
    ...
    case RejectReason::OrderNotCancellable:
        return "ORDER_NOT_CANCELLABLE";
    case RejectReason::InvalidModify:
        return "INVALID_MODIFY";
    case RejectReason::StopPriceInconsistent:
        return "STOP_PRICE_INCONSISTENT";
    case RejectReason::PriceBandViolation:
        return "PRICE_BAND_VIOLATION";
    case RejectReason::MaxSizeViolation:
        return "MAX_SIZE_VIOLATION";
    case RejectReason::MarketExhausted:
...
static events::OrderAction parse_action(const std::string& s) {
    if (s == "NEW_ORDER")
        return events::OrderAction::NewOrder;
    if (s == "CANCEL_ORDER")
        return events::OrderAction::CancelOrder;
    ...
    return std::visit(
        [](const auto& e) -> std::string {
            ...
            if constexpr (std::is_same_v<T, events::TradeExecutedEvent>) {
                json j{{"ts", e.timestamp},          {"type", "TRADE"},
                       {"symbol", e.symbol},         {"buy_id", e.buy_order_id},
                       {"sell_id", e.sell_order_id}, {"price", e.price},
                       {"qty", e.quantity}};
                return "TRADE " + j.dump();

            } else if constexpr (std::is_same_v<T, events::BookUpdatedEvent>) {
                json j{{"ts", e.timestamp},          {"type", "BOOK"},
                       {"symbol", e.symbol},         {"best_bid", e.best_bid},
                       {"best_ask", e.best_ask},     {"spread", e.spread},
                       {"bid_levels", e.bid_levels}, {"ask_levels", e.ask_levels}};
                return "BOOK " + j.dump();
                ...
                json j{{"ts", e.timestamp},      {"type", "SNAPSHOT"},
                       {"symbol", e.symbol},     {"last_trade_price", e.last_trade_price},
                       {"best_bid", e.best_bid}, {"best_ask", e.best_ask},
                       {"bids", bids},           {"asks", asks}};
                ...
                json j{{"ts", e.timestamp},  {"type", "REJECTED"},
                       {"symbol", e.symbol}, {"order_id", e.order_id},
                       {"cid", e.client_id}, {"reason", reject_reason_to_str(e.reason)}};
